...
int sem_insert(int sem, int count){
	...
	semtable[sem].waitlist.pid[semtable[sem].waitlist.count] = proc->pid;
	semtable[sem].waitlist.count++;

	return 0;
}

int sem_remove(int sem) {
	int i,j;
	int retval = 0;
...
int sem_wait(int sem, int count) {
	...
	if(semtable[sem].value >= count) {
		...
		sleep((void *)proc->pid, &semtable[sem].lock);
	}
